...
class OllamaBot:
    def __init__(self):
        ...
        self.main_document = Document(page_content="")
        self._load_content()
    ...
    def _load_content(self):
        """
        Load and process all .htm files from the base directory, merging them into a single Document.
        """
        htm_files = self._list_htm_files(".htm")
        print(f"Found {len(htm_files)} .htm files.")

        all_text = []  # Collect all document texts
        ...
        # Create a single Document containing all merged text
        merged_content = "\n\n".join(all_text) if all_text else "No content available."
        self.main_document = Document(page_content=merged_content)

        print("Document initialization is successful.")
    ...
    def query(self, question):
        ...
        llama_document = LlamaDocument(text=self.main_document.page_content) 
        
        if not os.path.exists("all_documents.pkl"):
            raw_nodes = node_parser.get_nodes_from_documents([llama_document])
        else:
            raw_nodes = pickle.load(open("all_documents.pkl"))
